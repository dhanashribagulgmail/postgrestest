...
def add_patient(request):
    ...
    if request.method=="POST":
        ...
        if  Patient.objects.filter(name=name).exists():
            messages.error(request ,'Patient is already exist')
            return redirect('/patient_data/add_new_patient/')
        ...
        messages.success(request ,'Patient is Saved Successfully')

        return redirect('/patient_data/')
    
    patients = Patient.objects.all()

    return render(request, "home/addnewPatient.html", {'patients': patients})   
